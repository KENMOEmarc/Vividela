...
import com.template.auth.config.LoyaltyProperties;
import com.template.auth.exception.ResourceNotFoundException;
import com.template.auth.model.dto.ArticleCreateRequest;
import com.template.auth.model.dto.DepositRequest;
import com.template.auth.model.dto.OrderCreateRequest;
import com.template.auth.model.dto.OrderDto;
import com.template.auth.model.dto.OrderUpdateRequest;
...
import com.template.auth.model.mapper.OrderMapper;
...
import com.template.auth.service.FeedbackService;
...
public class OrderServiceImpl implements OrderService {
	...
	@Lazy
	private final com.template.auth.service.ArticleService articleService;
	...
	public void recalculateStatus(Long orderId) {
		...
		boolean allPending = articles.stream()
				.allMatch(a -> a.getStatus() == com.template.auth.model.enums.ArticleStatus.PENDING);
		boolean allCompleted = articles.stream()
				.allMatch(a -> a.getStatus() == com.template.auth.model.enums.ArticleStatus.COMPLETED);
		...
		for (var articleDto : request.getArticles()) {
			ArticleCreateRequest articleRequest = ArticleCreateRequest.builder()
					...
					.status(articleDto.getStatus() != null ? articleDto.getStatus()
							: com.template.auth.model.enums.ArticleStatus.PENDING)
					.services(articleDto.getServices() != null
							? articleDto.getServices().stream()
									.map(com.template.auth.model.dto.ArticleServiceDto::getService)
	...
	private void processLoyaltyOnDelivery(Order order) {
		...
		BigDecimal earnedRaw = netPaid.multiply(loyaltyProperties.getEarnRatePerCurrencyUnit());
	...
	private BigDecimal computeNetAmountDue(Order order) {
		...
		int pointsUsed = order.getLoyaltyPointsUsed() != null ? order.getLoyaltyPointsUsed() : 0;
		if (pointsUsed > 0) {
			BigDecimal pointsValue = loyaltyProperties.getPointValue().multiply(BigDecimal.valueOf(pointsUsed));
	...
	private String lockedReason(OrderStatus status, PaymentStatus paymentStatus) {
		if (status == OrderStatus.DELIVERED) return "livrée";
		if (status == OrderStatus.CANCELLED) return "annulée";
		if (paymentStatus == PaymentStatus.COMPLETED) return "déjà payée intégralement";
		return "verrouillée";
	}
	...
	private OrderDto toDto(Order order, boolean includeArticles) {
		...
		Map<Long, List<ArticleService>> servicesByArticle = includeArticles
	...
	private List<OrderDto> toDtoList(List<Order> orders, boolean includeArticles) {
		...
		Map<Long, List<ArticleService>> servicesByArticle;
		...
		return orders.stream()
				.map(order -> {
					...
					Map<Long, List<ArticleService>> servicesForThisOrder = includeArticles
							? articles.stream().collect(Collectors.toMap(
									Article::getId,

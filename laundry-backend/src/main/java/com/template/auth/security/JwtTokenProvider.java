 ...
 *   jti  (JWT ID)     : identifiant unique du token (pour la blacklist Redis)
...
public class JwtTokenProvider {
    ...
    /**
     * Garde-fou au démarrage : en profil "prod", on refuse de démarrer
     * l'application si JWT_SECRET n'a pas été positionné (retombée sur la
     * valeur par défaut) ou s'il est trop court pour HS256. Voir revue de
     * code — "Secret JWT par défaut commité dans application.yml".
     */
    @PostConstruct
    void validateSecretOnStartup() {
        ...
        if (jwtSecret == null || jwtSecret.isBlank()) {
            throw new IllegalStateException(
                    "JWT_SECRET manquant. Définissez la variable d'environnement JWT_SECRET "
                            + "avec une clé aléatoire d'au moins 32 caractères avant de démarrer en profil 'prod'.");
        }
        if (jwtSecret.getBytes(StandardCharsets.UTF_8).length < MIN_SECRET_LENGTH) {
            throw new IllegalStateException(
                    "JWT_SECRET est trop court pour HS256 (256 bits minimum requis, soit "
                            + MIN_SECRET_LENGTH + " caractères ASCII au moins).");
        }
    }
    ...
    public String generateToken(Long userId, String username, String email) {
        ...
        return Jwts.builder()
                ...
                .subject(email)
                // JWT ID unique → permet de blacklister ce token précis dans Redis
     ...
     * Utilisée pour définir le TTL dans Redis lors de la blacklist.
     */

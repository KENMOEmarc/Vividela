...
import com.template.auth.model.entity.Stock;
...
public interface StockRepository extends JpaRepository<Stock, Long> {
    ...
    /** Premier lot d'un produit (utilisé pour la compatibilité avec l'ancien modèle 1-1). */
    Optional<Stock> findFirstByProductIdOrderByIdAsc(Long productId);
    ...
    /** Date d'expiration la plus proche parmi les lots encore approvisionnés (quantité > 0). */
    @Query("SELECT MIN(s.expirationDate) FROM Stock s WHERE s.product.id = :productId " +
           "AND s.currentQuantity > 0 AND s.expirationDate IS NOT NULL")
    LocalDate findNearestExpirationByProductId(@Param("productId") Long productId);

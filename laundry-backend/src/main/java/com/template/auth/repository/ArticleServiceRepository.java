...
public interface ArticleServiceRepository extends JpaRepository<ArticleService, Long> {

    List<ArticleService> findByArticleId(Long articleId);
     ...
     */
    List<ArticleService> findByArticleIdIn(List<Long> articleIds);
    ...
    @Query("DELETE FROM ArticleService asv WHERE asv.article.id = :articleId")
     ...
     */
    @Query("SELECT COALESCE(SUM(asv.appliedPrice), 0) FROM ArticleService asv WHERE asv.article.order.id = :orderId")
     ...
     */
    @Query("SELECT COUNT(asv) > 0 FROM ArticleService asv WHERE asv.article.clothingType = :clothingType AND asv.service = :service")
    boolean existsByClothingTypeAndService(@Param("clothingType") com.template.auth.model.enums.ClothingType clothingType,
                                            @Param("service") com.template.auth.model.enums.ServiceType service);
}
